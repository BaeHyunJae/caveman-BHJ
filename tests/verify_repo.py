...
def verify_manifests_and_syntax() -> None:
    ...
    # The default scan turns EVERY .md under agents/ into a subagent, named from
    # its frontmatter or filename, and it recurses into subdirectories: measured
    # on Claude Code 2.1.237, agents/docs/zz-probe.md registers as the subagent
    # `docs:zz-probe`. agents/AGENTS.md and agents/CLAUDE.md (maintainer docs for
    # the profile registry) shipped as bogus subagents named AGENTS and CLAUDE;
    # they now live in docs/agents/, outside agents/ altogether.
    ...
    agent_md = {
        ...
        agent_md == expected_agent_files,
        "agents/**/*.md is scanned wholesale into every user's subagent list; "
        f"unexpected files ship as subagents: {sorted(agent_md - expected_agent_files)}. "
        "Move non-agent markdown out of agents/ — nesting renames the entry, not hides it.",
    )
