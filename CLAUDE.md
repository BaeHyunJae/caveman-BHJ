...
Consequence: **every `.md` under `agents/` becomes a subagent**, named from its
frontmatter or filename. The scan **recurses into subdirectories** — measured on
Claude Code 2.1.237, a file at `agents/docs/zz-probe.md` registers as the
subagent `docs:zz-probe`, so nesting renames the entry rather than hiding it.
`agents/AGENTS.md` and `agents/CLAUDE.md` shipped as bogus subagents called
`AGENTS` and `CLAUDE`; they now live in `docs/agents/`, outside `agents/`
altogether, which is the placement that removes them from the list. Put
non-agent markdown there. `tests/verify_repo.py` fails the build on either
regression.
